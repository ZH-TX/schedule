<template>
  ...
  <div class="page">
    ...
    <popup v-show="isShow" :option.sync="selectList" @change="handleShow" @modify="modify"  @del="del" />
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    //时间格式处理
    showTime(item){//item.start, item.len
      // console.log(event);
      
      let min=(m)=>{return ((m-1+9)%1===0)?'00':((m-1+9)%1)*60}
      let hour=(h)=>{return ((h-1+9)%1===0)?(h-1+9):(h-1+9)-(h-1+9)%1}
      item.time=`${hour(item.start)}:${min(item.start)}-${hour(item.start+item.len)}:${min(item.start+item.len)}`
      // console.log(item.time);
      
      return `${hour(item.start)}:${min(item.start)}-${hour(item.start+item.len)}:${min(item.start+item.len)}`
    ...
    changeStyle(item,index){
     return `margin-left:${(item.day-1)*160}px;margin-top:${(item.start-1)*60+2}px;height:${item.len*60-5}px;background-color:${this.colorArrays[index%9]}`
    }
  }
};
</script>
